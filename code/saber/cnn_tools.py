...
from keras.layers import Input, Conv2D, MaxPooling2D, UpSampling2D, Dropout, Conv2DTranspose
...
def pixelwise_crossentropy_loss_w(y_true, y_hat):
    y_hat += 1e-8   # avoid issues with log
    w = tf.cast(K.sum(y_true), tf.float32) / tf.cast(K.prod(K.shape(y_true)), tf.float32)
    ce = -(1 - w) * y_true * tf.math.log(y_hat) - w * (1. - y_true) * tf.math.log(1 - y_hat)
    return K.mean(ce)
...
def train_model(X_train, Y_train, X_valid, Y_valid, model, output_dir,
    ...
    for ii in trange(n_epochs, desc='Training Progress'):
    ...
    f1_val = f1_score(Y_valid, Yi_hat)    
    with tf.Session() as sess:
        f1_scalar = f1_val.eval()
            
    print('f1 on validation data:    %0.3f' % f1_scalar)
